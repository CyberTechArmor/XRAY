...
--
-- Idempotency: this migration mutates data (not just DDL). It's
-- re-run by the every-deploy migration loop, and the original form
-- was not idempotent — on the second run, 'admin' and 'viewer' are
-- both gone, but Step 2's subquery `(SELECT id FROM roles WHERE
-- slug='viewer')` returns NULL, and the UPDATE's SET role_id = NULL
-- hits the NOT NULL constraint on users.role_id. Wrapping the whole
-- thing in a DO block that short-circuits when neither 'admin' nor
-- 'viewer' still exists makes the migration a no-op on every run
-- after the first.
...
DO $$
BEGIN
  -- Early-exit guard: if neither 'admin' nor 'viewer' roles still
  -- exist, this migration has already run. Nothing to do.
  IF NOT EXISTS (SELECT 1 FROM platform.roles WHERE slug IN ('admin', 'viewer')) THEN
    RETURN;
  END IF;
