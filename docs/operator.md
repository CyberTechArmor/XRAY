...
bootstrap user to complete the strip (run from the directory
...
Note: use `docker compose exec postgres` (the compose service
name) rather than `docker exec <container-name>`. The container's
actual name is auto-generated by compose (e.g. `xray-postgres-1`)
and varies by project directory, but the service name is stable.
