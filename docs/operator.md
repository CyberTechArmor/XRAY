...
Migration 045 NOTICEs (rather than stripping) when the runtime
role IS the cluster bootstrap user. Postgres refuses to let the
bootstrap demote its own SUPERUSER attribute — that would orphan
the cluster (no one else has SUPERUSER to restore it). The error
is *"The bootstrap user must have the SUPERUSER attribute"*.

The default XRay docker stack falls into exactly this case: the
postgres image runs `initdb --username=xray`, making `xray` the
cluster bootstrap superuser. There's no separate `postgres` role
to fall back on. **The strip-superuser path is therefore a
dead-end on this install** — that's a Postgres invariant, not a
config bug. Run the role-split alternative below instead.

#### Diagnostic — confirm xray is the only superuser

```sh
cd /opt/xray  # or wherever docker-compose.yml lives
docker compose exec postgres psql -U xray -d xray -c \
  "SELECT rolname, rolsuper, rolbypassrls FROM pg_roles
    WHERE rolsuper OR rolbypassrls ORDER BY rolname;"
```

If `xray` is the only row → role-split is required.

#### Role-split — the actual production-ready path

Create a separate non-super `xray_app` role and switch the
application's `DATABASE_URL` to use it. xray remains as the
cluster bootstrap (admin / migrations / `pg_basebackup`); the
runtime application connection runs as a non-owner non-super
role, so RLS fires on every query.

1. **Create xray_app + grant DML on `platform.*`:**

   ```sh
   docker compose exec postgres psql -U xray -d xray <<'SQL'
     DO $$ BEGIN
       IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'xray_app') THEN
         CREATE ROLE xray_app WITH LOGIN PASSWORD '<pick-a-strong-password>'
           NOSUPERUSER NOINHERIT NOCREATEDB NOCREATEROLE;
       END IF;
     END $$;
     GRANT USAGE ON SCHEMA platform TO xray_app;
     GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA platform TO xray_app;
     GRANT USAGE, SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA platform TO xray_app;
     ALTER DEFAULT PRIVILEGES IN SCHEMA platform
       GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO xray_app;
     ALTER DEFAULT PRIVILEGES IN SCHEMA platform
       GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO xray_app;
   SQL
   ```

   Replace `<pick-a-strong-password>` with a random secret —
   store it in your secret manager.

2. **Update `.env`** to point the application at xray_app:

   ```sh
   # before:
   # DB_USER=xray
   # DB_PASSWORD=<old xray password>
   #
   # after:
   DB_USER=xray_app
   ...
   Keep the old xray password somewhere safe — you'll need it
   for `docker compose exec postgres psql -U xray …` admin
   operations going forward.

3. **Restart the server container** so it picks up the new
   `DATABASE_URL`:
...
4. **Verify RLS now fires for the application connection:**

   ```sh
   docker compose exec postgres psql -U xray_app -d xray -c \
     "SELECT current_user, rolsuper FROM pg_roles WHERE rolname=current_user;"
   # expect: xray_app | f
   ```

   Then sanity-check tenant isolation by running the SQL probe
   as `xray_app`:

   ```sh
   docker compose exec postgres psql -U xray_app -d xray \
     -f /tmp/probe-rls-cross-tenant.sql  # or pipe in via -i
   # expect: NOTICE: PROBE PASS — cross-tenant isolation holds
   ```

After step 3 completes, the application's runtime DB connection
is a non-owner non-super role, so the `tenant_isolation` and
`platform_admin_bypass` policies attached by migrations 029 + 044
fire on every query. RLS is genuinely enforcing in production.

#### Note: use `docker compose exec postgres`, not `docker exec <container-name>`

The container's actual name is auto-generated by compose
(e.g. `xray-postgres-1`) and varies by project directory and
compose version, but the **service** name (`postgres` per
`docker-compose.yml`) is stable. Always invoke via
`docker compose exec postgres …` from the directory containing
`docker-compose.yml`.
